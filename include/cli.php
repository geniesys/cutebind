<?php
    ...
    case 'status':
	echo '[STATUS] CuteBind is '.(($_pid and posix_kill($_pid,SIGHUP)) ? '':'NOT ').'running ('.$pidfile.").\n";
	break;

    case 'fullstatus':
	if (! $_pid) {
	    echo "[DEBUG] CuteBind process id is not found. Have you ever run it?\n";
	    break;
	}
	...
	if (file_exists($settings['intfile'])) unlink($settings['intfile']);		// delete previous report, if found.
	...
	if (posix_kill($_pid,SIGUSR1)) {
		...
		$ok = FALSE;
		for($i = 0; $i < 30; $i++) {
		    if(file_exists($settings['intfile'])) $ok = TRUE; break;
		    usleep(500000);
		}
		if ($ok) {
		    echo "OK.\n".file_get_contents($settings['intfile'])."\n\n";
		    unlink($settings['intfile']);					// delete report after is has been displayed.
		} else {
		    echo "FAILED.\n\n";
		    echo 'File '.$settings['intfile']." does not exist or access denied. Permissions?\n";
		}
	...
	if ($_pid && posix_kill($_pid,SIGHUP)) {
	    echo 'ERROR. CuteBind with pid-file \''.$pidfile.'\' is running alredy (PID '.$_pid.")\n";
	} else {
	    shell_exec($startcmd);
	    $i = 0;
	    $ok = TRUE;
	...
	echo "Usage: cutebind (start|stop|restart|fullstatus|status|configtest|help)\n";
	echo "\tAlso you can use some optional parameters to override the same config variables.\n";
	echo "\t--pid-file='/path/to/pid-file'	- Pid-file\n";
	echo "\t--listen='127.0.0.1'		- Hosts of the DNS-server. localhost is default - Comma separated list.\n";
	echo "\t--listen-port=53		- Default port of the DNS-server, default: 53\n";
	echo "\t--cutebind-path='cutebind'	- You can set a path to CuteBind's sh-file\n";
	echo "\t--config-file='/path/to/config.php'  - You can set a path to configuration file manually\n";
	echo "\t--logging=1			- Logging. 1-Enable, 0-Disable\n";
	echo "\t--log-storage='/path/to/logs/log%DATE=d.m.Y%.log'  - Log storage. This field has special syntax, but it allows simple path.\n";
	echo "\t--set-user=cutebind		- You can set user of master process (aka sudo).\n";
	echo "\t--set-group=cutebind		- You can set user of master process (aka sudo).\n";
	echo "\t--int-file=			- This path must be a point to file in writable folder for all CuteBind processes.\n";
